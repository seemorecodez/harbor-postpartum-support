...
| Public source and CI | **Conditional** | Public alpha.28 verification commit `e8833c1` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 117 tests including the 1,000-record widget workload, the separately governed encrypted 1,000-record/20-migration workload, root and Pages-subpath release WebAssembly builds, 49-asset/9-precache finalization, clean-tree verification, artifact preservation, deterministic 64-component CycloneDX generation, SLSA provenance plus CycloneDX Sigstore attestations, and least-privilege Pages deployment. The exact downloaded CI artifact hashes to the digest bound by both attestations, and the signed CI SBOM is byte-identical to local recomputation. The canonical alpha.28 feature web/source packages remain reconciled to `55ad236` and match 49/49 CI files plus 216/216 raw Git blobs; later commits change tests, CI, supply-chain controls, and evidence only. Alpha.25's manual native run still passes Android, Windows, and Apple hosted compilation with package/privacy assertions. Long-term artifact retention, independent dependency/license/vulnerability review, installation/runtime evidence, signed native artifacts, and cross-OS deterministic web output remain |
| Web product | **Conditional** | Versioned alpha.28 PWA source, public Pages deployment, and finalized root/subpath artifacts exist. Alpha.28 adds production local journal search, lazy large-history rendering, controller-bound persistent destination screens, and governed production-vault/widget workloads across 1,000 combined records. The latest public Linux gate measures 76.872 ms widget navigation and 41.102 ms worst widget search; the live origin returns alpha.28/build 28. A fresh live exercise completed onboarding and the full semantic shell with an empty console, and five new-tab launches reached the shell in 597–709 ms. Two bounded Flutter Chrome 152 test-launcher attempts emitted no test event, including under Xvfb, so browser-compiled frame timing remains open. Earlier verified browser paths cover upgrades, encrypted schema migration/restart, locked recovery, visible online/offline startup, fatal-load recovery, responsive and accessible layouts, encrypted synthetic data, care tools, factual reflections, clinician-question handoff, transparent offline Stories, truthful About/content versions, bounded diagnostics, the nine-scenario clinical-safety guide, urgent-support routing, erase, app lock, exact-preview/cancel privacy, corrupt-update recovery, repository-subpath online/offline loading, and cross-origin frame refusal. Release assets are pinned by SHA-256 and length and verified before cache promotion; the browser-installed worker remains the manifest trust root. GitHub Pages lacks Harbor's desired CSP/frame/nosniff response headers. A header-controllable production host, supported-browser update/offline/performance timing, host logging/retention, hostile same-origin releases, quota/eviction, non-Chromium rollback, cold launch, and baseline devices remain open. Native clipboard/crash/recents forensics, device credentials, and app-switcher proofs remain open |
...
The React Native ZIP remains **prototype source**. The current Flutter worktree and public deployment are alpha.28 at verification commit `e8833c1`; the latest reconciled canonical feature downloads remain alpha.28 at `55ad236`. The latest short-retention CI web artifact has verified Sigstore provenance and SBOM attestations, but the canonical downloads and all native outputs are not thereby retroactively signed. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
