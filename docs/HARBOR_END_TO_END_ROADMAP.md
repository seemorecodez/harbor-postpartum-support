...
| Public source and CI | **Conditional** | Public `main` has an immutable-action Ubuntu 24.04 clean-checkout gate; the latest verified public alpha.17 revision passes Flutter 3.47.2 lockfile resolution, formatting, analysis, all 53 tests, release WebAssembly build, clean-tree verification, and short-lived artifact preservation. The local alpha.18 diagnostic slice passes analysis, all 57 tests, and release WebAssembly compilation; its public CI/package evidence is pending this commit. Two clean Ubuntu alpha.17 builds produced 49/49 identical application files; native CI, signing/provenance, long-term retention, and cross-OS deterministic output remain |
| Web product | **Conditional** | Versioned alpha.18 PWA builds locally; verified browser paths cover upgrades, encrypted schema migration/restart, locked recovery, visible online/offline startup, fatal-load recovery, desktop/phone responsive layouts, keyboard onboarding/navigation, semantic exposure, encrypted synthetic data, care tools, factual reflections, clinician-question handoff, transparent offline Stories with encrypted private resonance, truthful About/content versions, manual five-field diagnostic preview/cancel, urgent-support routing, and erase; full feature/release gates remain |
...
| Clinical approval | **Fail** | Controlled source/content registry exists but lacks named clinical and lived-experience sign-off |
...
The React Native ZIP remains **prototype source**. The committed Flutter tree is alpha.18; the current integrity-audited public web/source downloads remain alpha.17 until alpha.18 completes public CI and package verification. Both remain **engineering alpha**, not a “complete app” or release candidate; no native release artifact is implied by the presence of native target source.
