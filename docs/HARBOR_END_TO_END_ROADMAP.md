...
| Public static privacy-policy URL | Implemented locally in alpha.29; public deployment and counsel review pending | Store rejection or misleading claims | Pass the public gate, then obtain legal/privacy approval and host-policy acceptance | Store release blocked |
...
| Public source and CI | **Conditional** | Public alpha.28 verification commit `351102a` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 126 tests including the 1,000-record widget workload, the separately governed encrypted 1,000-record/20-migration workload, root and Pages-subpath release WebAssembly builds, 49-asset/9-precache finalization, clean-tree verification, artifact preservation, deterministic license-enriched 64-component CycloneDX generation, a byte-identical cross-OS 65-component license inventory, SLSA provenance plus CycloneDX Sigstore attestations, and least-privilege Pages deployment. A release-blocking OSV v2.5.1 scan parsed all 78 locked packages and reported no known issues before deployment. The exact downloaded CI artifact hashes to the digest bound by both attestations; the signed CI SBOM and inventory are byte-identical to Windows recomputation. The canonical alpha.28 feature web/source packages remain reconciled to `55ad236` and match 49/49 CI files plus 216/216 raw Git blobs; later commits change tests, CI, supply-chain controls, and evidence only. Alpha.25's manual native run still passes Android, Windows, and Apple hosted compilation with package/privacy assertions. Long-term artifact retention, independent source/license/application-security review, installation/runtime evidence, signed native artifacts, and cross-OS deterministic web binary output remain |
| Web product | **Conditional** | Versioned alpha.29 PWA source and finalized root/subpath artifacts exist locally; the public Pages origin remains alpha.28 pending the alpha.29 gate. Alpha.29 adds a matching in-app/static privacy notice, explicit host-metadata and erasure limitations, an exact worker navigation boundary, and a required integrity-pinned offline asset. All 132 tests, both 50-asset/10-precache release builds, the 1,000-record workloads, a direct compiled privacy-page inspection, worker-controlled navigation, and server-stopped privacy reload pass locally. The page exposes one main landmark, one level-one heading, ten labeled regions, and no scripts, forms, images, subresources, overflow, or warning/error logs. The latest public alpha.28 Linux gate measures 84.922 ms widget navigation and 36.311 ms worst widget search; one older local run exceeded the threshold, while the current full alpha.29 Windows run passes at 143.991 ms and 58.817 ms, so load stability remains open. GitHub Pages lacks Harbor's desired CSP/frame/nosniff response headers and retains ordinary request metadata outside Harbor's control. A public alpha.29 exercise, legal/privacy approval, header-controllable production host, supported-browser update/offline/performance timing, host logging/retention, hostile same-origin releases, quota/eviction, non-Chromium rollback, cold launch, controlled-load performance, and baseline devices remain open. Native clipboard/crash/recents forensics, device credentials, and app-switcher proofs remain open |
...
| Store/legal readiness | **Fail** | A locally verified engineering-alpha privacy notice now exists, but public deployment, accountable operator/contact details, store declarations, terms, counsel, and accounts are incomplete |

The React Native ZIP remains **prototype source**. The current Flutter worktree is alpha.29; the public deployment and latest reconciled canonical feature downloads remain alpha.28 until the alpha.29 public gate passes. The latest short-retention alpha.28 CI web artifact has verified Sigstore provenance and a license-enriched SBOM attestation after both the full verify job and immutable OSV gate passed, but the canonical downloads and all native outputs are not thereby retroactively signed. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
