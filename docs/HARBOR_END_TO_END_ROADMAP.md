...
| Public source and CI | **Conditional** | Public alpha.25 commit `b19b83f` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 105 tests including strict Linux goldens/native contracts, release WebAssembly build, 49-asset/9-precache finalization, clean-tree verification, and web-artifact preservation. Its manual native run also passes Android, Windows, and Apple hosted jobs with compiled-product/privacy assertions and short-lived engineering-proof artifacts. Signing/provenance, installation/runtime evidence, long-term retention, cross-OS deterministic web output, and downloadable-package reconciliation remain |
