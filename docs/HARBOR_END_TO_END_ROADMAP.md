...
The React Native ZIP remains **prototype source**. The current Flutter worktree is alpha.28; the latest reconciled canonical downloads and public deployment remain alpha.27 at tested feature commit `cbde264` until the alpha.28 public gate passes. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
