...
| Public source and CI | **Conditional** | Public alpha.27 commit `cbde264` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 109 tests, root and Pages-subpath release WebAssembly builds, 49-asset/9-precache finalization, clean-tree verification, artifact preservation, and least-privilege Pages deployment. Its canonical web/source packages match 49/49 CI files and 211/211 raw Git blobs. Alpha.25's manual native run still passes Android, Windows, and Apple hosted compilation with package/privacy assertions. Signing/provenance, installation/runtime evidence, long-term retention, and cross-OS deterministic web output remain |
| Web product | **Conditional** | Versioned alpha.27 PWA source, public Pages deployment, and separately finalized repository-subpath artifact exist. Verified browser paths cover upgrades, encrypted schema migration/restart, locked recovery, visible online/offline startup, fatal-load recovery, responsive and accessible layouts, encrypted synthetic data, care tools, factual reflections, clinician-question handoff, transparent offline Stories, truthful About/content versions, bounded diagnostics, the nine-scenario clinical-safety guide, urgent-support routing, erase, the alpha.20 passphrase-wrapped app lock paths, alpha.21/22 compiled accessibility smokes, alpha.23 exact-preview/cancel privacy paths, alpha.24 corrupt-update recovery, alpha.26 public/subpath online-offline loading, and alpha.27 local/public cross-origin frame refusal. Release assets are pinned by SHA-256 and length and verified before cache promotion; the browser-installed worker remains the manifest trust root. GitHub Pages lacks Harbor's desired CSP/frame/nosniff response headers, and alpha.26 was demonstrably frameable. Alpha.27's client boundary is defense-in-depth; the first public retry remained on completed alpha.26 until worker promotion, after which the hostile frame showed only the safety boundary. A header-controllable production host, supported-browser update/offline timing, host logging/retention, hostile same-origin releases, quota/eviction, and non-Chromium rollback remain open. Native clipboard/crash/recents forensics remain open. Automatic web locking remains dependent on visibility/focus events supplied by the host; native device-credential and app-switcher proofs remain open |
...
The React Native ZIP remains **prototype source**. The current Flutter worktree and latest reconciled canonical downloads are alpha.27 at tested feature commit `cbde264`. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
