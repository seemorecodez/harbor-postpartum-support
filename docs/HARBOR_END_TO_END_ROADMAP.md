...
| Encrypted local database | Shared AES-256-GCM vault and migration proof exist; native/plugin/key-custody proof incomplete | Sensitive local data exposure | Complete installed-target storage, key-custody, and forensic proof | Do not release native apps |
| Independent security review | Missing | Unknown privacy/security defects | Budget and schedule assessor | No public release |
| Public static privacy-policy URL | Live at the public alpha.29 Pages URL with exact-claim tests and browser inspection; counsel and production-host review pending | Store rejection or misleading claims | Obtain legal/privacy approval and host-policy acceptance | Store release blocked |
...
| Public source and CI | **Conditional** | Public alpha.29 recovery-operations run `33671959347` passes the immutable-action Ubuntu 24.04 clean-checkout gate at exact commit `59309cc46fbfba7885e924debe5d1e0a4b7c3182`: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 132 Flutter tests, the separate three-test recovery-verifier gate, both governed 1,000-record workloads, root and Pages-subpath release WebAssembly builds, 50-asset/10-precache finalization, clean-tree verification, artifact preservation, deterministic license-enriched 64-component CycloneDX generation, the 65-component license inventory, SLSA provenance plus CycloneDX Sigstore attestations, and least-privilege Pages deployment. A release-blocking OSV v2.5.1 scan parsed all 78 locked packages and reported no known issues before deployment. The durable public alpha.29 engineering prerelease at tag `v0.1.0-alpha.29` preserves the exact attested 50-file CI artifact, all 228 source blobs from `75555a4`, SBOM, inventory, attestation bundles, checksums, release notes, and roadmap. A fresh release download passes the fail-closed recovery verifier and both public attestation predicates without reading personal content. Alpha.25's manual native run still passes Android, Windows, and Apple hosted compilation with package/privacy assertions. Independent archive mirroring, source/license/application-security review, installation/runtime evidence, signed native artifacts, and cross-OS deterministic web binary output remain |
| Web product | **Conditional** | Versioned alpha.29 PWA source and public Pages deployment now include a matching in-app/static privacy notice, explicit host-metadata and erasure limitations, an exact worker navigation boundary, and a required integrity-pinned offline asset. All 132 public Flutter tests, all three recovery tests, both 50-asset/10-precache release builds, both 1,000-record workloads, direct compiled privacy-page inspection, worker-controlled navigation, and server-stopped privacy reload pass. The page exposes one main landmark, one level-one heading, ten labeled regions, and no scripts, forms, images, subresources, overflow, or warning/error logs. Latest public alpha.29 Linux measures 75.217 ms widget navigation, 38.992 ms worst widget search, 134.679 ms encrypted save, 28.656 ms reload, 83.134 ms worst migration, and 1.322 ms worst production search. A browser still controlled by alpha.28 initially received that worker's app-shell fallback for `privacy.html`; after alpha.29 promoted and one reload, the complete notice rendered. Fresh network fetches are correct immediately and alpha.29 fixes future routing, but supported-browser promotion timing remains open. GitHub Pages lacks Harbor's desired CSP/frame/nosniff response headers and retains ordinary request metadata outside Harbor's control. Legal/privacy approval, a header-controllable production host, host logging/retention, hostile same-origin releases, quota/eviction, non-Chromium rollback, cold launch, controlled-load performance, and baseline devices remain open. Native clipboard/crash/recents forensics, device credentials, and app-switcher proofs remain open |
| Desktop product | **Fail** | Public hosted toolchains compile Windows and macOS release trees; Windows additionally verifies one unsigned `Harbor.exe`, its Flutter assets, and clean source. No package has passed installation or runtime behavior on a clean consumer system |
| Production architecture | **Conditional** | Shared Flutter web runtime, Windows encrypted-store library proof, and Android/Windows/macOS/iOS hosted compilation pass. Android's merged APK manifest additionally passes backup, cleartext, INTERNET-permission, and product-name assertions. All native install, runtime, device-key, privacy-forensic, and distribution proofs remain |
| Encrypted personal-content storage | **Conditional** | Production AES-256-GCM shared vault, version-1/2-to-version-3 migration/restart, interrupted-write recovery, future-version lockout, and Windows sqlite3mc proof pass. Alpha.28 saves/reloads 1,000 exact records and verifies 20 independent legacy migration/restart cycles (20,000 migrated record instances) without staging residue or synthetic plaintext in encrypted envelopes. Plugin-backed/native storage, platform key custody, destructive fault injection, and forensic reviews remain |
...
| Store/legal readiness | **Fail** | A publicly deployed, mechanically verified engineering-alpha privacy notice now exists, but accountable operator/contact details, store declarations, terms, counsel, host acceptance, and accounts are incomplete |
| Operations and recovery | **Fail** | A detailed privacy-preserving incident/correction/forward-rollback/credential-compromise/sunset runbook exists, and a fresh download of the durable alpha.29 prerelease passes its executable known-good-anchor drill. Actual public correction/rollback, credential rotation, support communications, native recovery, sunset, named ownership, and independent observation remain unperformed |

The React Native ZIP remains **prototype source**. The current Flutter application and public deployment are alpha.29; the durable `v0.1.0-alpha.29` engineering prerelease is anchored to exact commit `75555a498fea9183578c0927a3cd0af56f5d61fd` and the matching attested public-CI web artifact. Its source, SBOM, license inventory, checksums, attestations, release notes, and roadmap are reconciled in the public release. The later recovery-operations checkpoint passes publicly at `59309cc46fbfba7885e924debe5d1e0a4b7c3182`. Native outputs are not signed or included. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or downloadable web artifact is not an installed, clinically approved, or signed native release.
