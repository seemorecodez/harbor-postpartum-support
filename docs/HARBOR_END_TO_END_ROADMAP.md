...
The React Native ZIP remains **prototype source**. The committed Flutter tree is alpha.24; the current integrity-audited public web/source downloads remain alpha.23 until the alpha.24 public clean-checkout gate and package comparison pass. They remain **engineering alpha**, not a “complete app” or release candidate; no native release artifact is implied by the presence of native target source.
