...
| Public source and CI | **Conditional** | Public alpha.25 commit `b19b83f` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 105 tests including strict Linux goldens/native contracts, release WebAssembly build, 49-asset/9-precache finalization, clean-tree verification, and web-artifact preservation. Its manual native run also passes Android, Windows, and Apple hosted jobs with compiled-product/privacy assertions and short-lived engineering-proof artifacts. The web package matches all 49 canonical CI files and the source package matches all 210 raw Git blobs. Signing/provenance, installation/runtime evidence, long-term retention, and cross-OS deterministic web output remain |
| Web product | **Conditional** | Versioned alpha.26 PWA source and a separately finalized repository-subpath artifact exist. Verified browser paths cover upgrades, encrypted schema migration/restart, locked recovery, visible online/offline startup, fatal-load recovery, responsive and accessible layouts, encrypted synthetic data, care tools, factual reflections, clinician-question handoff, transparent offline Stories, truthful About/content versions, bounded diagnostics, the nine-scenario clinical-safety guide, urgent-support routing, erase, the alpha.20 passphrase-wrapped app lock paths, alpha.21/22 compiled accessibility smokes, alpha.23 exact-preview/cancel privacy paths, alpha.24 corrupt-update recovery, and alpha.26 fresh-origin subpath online/offline loading. Release assets are pinned by SHA-256 and length and verified before cache promotion; the browser-installed worker remains the manifest trust root. The public Pages deployment, its headers/logging, hostile same-origin releases, quota/eviction, and non-Chromium rollback remain open. Native clipboard/crash/recents forensics remain open. Automatic web locking remains dependent on visibility/focus events supplied by the host; native device-credential and app-switcher proofs remain open |
...
The React Native ZIP remains **prototype source**. The current Flutter worktree is alpha.26; the latest canonical downloads remain alpha.25 until alpha.26 passes public CI/deployment and package reconciliation. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
