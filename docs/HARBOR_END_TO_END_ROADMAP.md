...
| Public source and CI | **Conditional** | Public alpha.28 commit `0b3d5a4` passes the immutable-action Ubuntu 24.04 clean-checkout gate: Flutter 3.47.2 lockfile resolution, formatting, analysis, all 113 tests including the 1,000-record widget workload, the separately governed encrypted 1,000-record/20-migration workload, root and Pages-subpath release WebAssembly builds, 49-asset/9-precache finalization, clean-tree verification, artifact preservation, and least-privilege Pages deployment. The canonical alpha.28 feature web/source packages remain reconciled to `55ad236` and match 49/49 CI files plus 216/216 raw Git blobs; later commits change tests, CI, and evidence only. Alpha.25's manual native run still passes Android, Windows, and Apple hosted compilation with package/privacy assertions. Signing/provenance, installation/runtime evidence, long-term retention, and cross-OS deterministic web output remain |
...
The React Native ZIP remains **prototype source**. The current Flutter worktree and public deployment are alpha.28 at verification commit `0b3d5a4`; the latest reconciled canonical feature downloads remain alpha.28 at `55ad236`. All remain **engineering alpha**, not a “complete app” or release candidate; a public static deployment or hosted compile artifact is not an installed, clinically approved, or signed native release.
