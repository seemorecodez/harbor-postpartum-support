...
| Public source and CI | **Conditional** | Public `main` has an immutable-action Ubuntu 24.04 clean-checkout gate; Flutter 3.47.2 lockfile resolution, formatting, analysis, all 53 tests, release WebAssembly build, clean-tree verification, and short-lived artifact preservation pass. Two clean Ubuntu alpha.17 builds produced 49/49 identical application files, and web/source downloads are integrity-audited; native CI, signing/provenance, long-term retention, and cross-OS deterministic output remain |
...
The React Native ZIP remains **prototype source**. The committed Flutter tree and current integrity-audited web/source downloads are alpha.17. They remain **engineering alpha**, not a “complete app” or release candidate; no native release artifact is implied by the presence of native target source.
