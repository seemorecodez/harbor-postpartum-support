...
- **Verification:** Alpha.29 adds six tests covering exact cross-surface claims, a no-tracking/CSP contract, structural accessibility, exact worker routing, in-app vault exclusion, and 200%-text accessibility. The 132-test local/public gates, both 50-asset/10-precache WebAssembly finalizations, direct local/public page inspection, worker-controlled online navigation, and server-stopped reload pass with an empty browser warning/error log. One alpha.28-controlled public profile needed worker promotion and one reload before the old app-shell fallback stopped intercepting the new privacy route; the alpha.29 worker fixes future exact routing.
- **Remaining gate:** Obtain qualified privacy/legal review; select a host with accepted response headers and logging/retention; publish accountable contact/controller details if counsel requires them; bound promotion on supported browsers; and repeat on signed native packages. This engineering disclosure is not a legal approval or privacy certification.

### HBR-DEC-028 — Rollback uses a durable verified anchor and a new forward release

- **Decision:** Harbor's known-good web recovery anchor is a durable public engineering prerelease containing the exact attested CI web ZIP, exact source archive, checksums, deterministic SBOM and license inventory, both Sigstore bundles, release notes, and roadmap snapshot. A rollback never reuses an old application/cache identity or blindly redeploys an old binary; it is a newly versioned forward corrective release that preserves later compatible migrations and privacy/security fixes and passes every current gate.
- **Reason:** Seven-day CI artifacts can expire during an incident, locally rebuilt web bytes differ by operating system, and an old worker/schema/content bundle can reintroduce a corrected hazard or strand encrypted data. A verified reference plus a new release identity makes the recovery decision reviewable without pretending old evidence approves new bytes.
- **Privacy boundary:** The verifier reads only public release artifacts and Git object bytes. It explicitly does not read a Harbor vault, browser profile, clipboard, crash report, or user identifier. Incident evidence is limited to public release identifiers, hashes, bounded error codes, synthetic test data, Harbor-controlled timestamps, and decisions.
- **Verification:** Public prerelease `v0.1.0-alpha.29` preserves nine assets at commit `75555a498fea9183578c0927a3cd0af56f5d61fd`. The exact web ZIP hashes to `D3BF0F8A89A1DB979F5B0393AF8DBFBCE915EE7DE02278828E5BB9C6AD7579E3`; its extracted worker verifies 50 manifest assets and 10 precache assets. The source ZIP hashes to `722007519213E38E61893B84553E13F15066D4D6BE4A3CA3A101C37F6661B299` and matches all 228 Git blobs. A fresh public-release download passes checksum, traversal, source, SBOM/license, structural-bundle, and online SLSA/CycloneDX attestation checks with `personalContentRead: false`. The first Windows source-archive candidate failed 54 blob comparisons and was quarantined.
- **Remaining gate:** Execute a real public forward content correction and rollback across every supported browser; rehearse credential revocation/rotation, native recovery, store removal, support communications, and sunset with named owners and independent observers. The verified anchor is not itself an executed rollback or production approval.
