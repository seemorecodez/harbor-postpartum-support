...
**Scope:** Harbor alpha 27 private vault, passphrase-wrapped local app lock, typed reviewed clipboard boundary, bounded diagnostics, integrity-pinned offline web/PWA updates, and fail-closed client framing boundary. This is an engineering threat model, not an independent security assessment or security guarantee.
...
| TM-002 | Host logs or network observers link a woman to Harbor use | High privacy harm | No personal-content request, referrer policy `no-referrer`, no analytics or third-party runtime host. Alpha.26's local subpath probe observed only same-origin GETs with no bodies or sentinel hits, and the public runtime loaded only same-origin application resources | GitHub Pages and network intermediaries can still observe IP/time/path metadata; Harbor cannot configure or inspect GitHub's underlying request-log retention. Publish the limitation, repeat supported-browser captures, and retain a self-hostable canonical build |
...
| TM-013 | Android cloud backup or device-to-device transfer silently copies the encrypted vault, wrapped key metadata, or preferences | Critical disclosure and unintended durable copies | The production manifest sets `allowBackup=false`; Android 11-and-lower backup rules and Android 12+ cloud/transfer rules exclude root, file, database, shared preferences, external, and device-protected variants. Source-contract tests pass and alpha.25 CI inspects the merged APK manifest | Framework/OS behavior and exclusion coverage still need physical-device cloud-backup and transfer attempts plus post-uninstall forensic inspection; other platforms need equivalent specifications |
...
| TM-008 | Another site frames Harbor and tricks a woman into activating controls | High privacy/safety harm | Alpha.26 public testing proved GitHub Pages frameable. Alpha.27 checks its top-level context before Flutter starts and, when framed, exposes only an assertive privacy notice plus a direct-open action; real local and public cross-origin probes confirm this after the update promotes. A production-style local host separately proves the desired `frame-ancestors 'none'` and X-Frame-Options response policy | The first public retry remained on completed alpha.26 until worker promotion. Client JavaScript can be removed by a malicious same-origin release and is not equivalent to browser-enforced response headers. Measure update timing across supported browsers, require a header-controllable production host, and obtain independent clickjacking review |
| TM-009 | Build or action dependency is replaced | Critical supply-chain compromise | Locked Dart dependencies; GitHub actions pinned to immutable commits; public clean-checkout CI; deterministic 64-component CycloneDX SBOM; verified Sigstore SLSA provenance and SBOM attestations for one exact public CI web artifact, workflow, and commit | The attested artifact/evidence expire after seven days; canonical downloads and native artifacts are not attested. Add durable release retention, independent dependency/license/vulnerability review, native signing, key-compromise drills, and a review cadence |
...
| Runtime assets do not depend on third-party hosts | Bundled fonts/assets, local HTML attributes, CSP/source contract, deterministic locked runtime-dependency SBOM attached to the exact CI web artifact | Public-host network capture plus independent dependency, license, and vulnerability review |
...
| Web app cannot expose private UI when embedded by another origin | Alpha.26 public failure is recorded; alpha.27's pre-Flutter boundary shows only a direct-open notice in real local and public cross-origin frames after update promotion; a local production-style response separately verifies `frame-ancestors 'none'` and X-Frame-Options | Require browser-enforced response headers on the production host and test security-update convergence; client code alone is defense-in-depth |
...
This model does not claim protection from a fully compromised operating system, browser, device administrator, malicious accessibility service, screen camera, physical coercion, offline guessing of a weak passphrase, an embedded browser that withholds lifecycle/focus signals, or a woman deliberately exporting/copying information. It does not cover a community backend because none is authorized or implemented. A GitHub Pages engineering deployment does not approve clinical content, the host as a production privacy boundary, signed native applications, or store release.
