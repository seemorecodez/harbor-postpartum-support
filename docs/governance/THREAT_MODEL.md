...
| TM-008 | Another site frames Harbor and tricks a woman into activating controls | High privacy/safety harm | Alpha.26 public testing proved GitHub Pages frameable. Alpha.27 checks its top-level context before Flutter starts and, when framed, exposes only an assertive privacy notice plus a direct-open action; a real cross-origin local probe confirms this behavior. A production-style local host separately proves the desired `frame-ancestors 'none'` and X-Frame-Options response policy | Client JavaScript can be removed by a malicious same-origin release and is not equivalent to browser-enforced response headers. Pass the alpha.27 public hostile-frame retest, then require a header-controllable production host and independent clickjacking review |
...
| Web app cannot expose private UI when embedded by another origin | Alpha.26 public failure is recorded; alpha.27's pre-Flutter boundary shows only a direct-open notice in a real local cross-origin frame; a local production-style response separately verifies `frame-ancestors 'none'` and X-Frame-Options | Pass the alpha.27 public hostile-frame retest and require browser-enforced response headers on the production host; client code alone is defense-in-depth |
