...
- [Public GitHub clean-checkout verification](https://github.com/seemorecodez/harbor-postpartum-support/actions/runs/33428694816) for alpha.21 on Ubuntu 24.04 repeats locked dependency resolution, formatting, static analysis, all 87 tests, the release WebAssembly build, a post-build clean-tree check, and artifact preservation using immutable action revisions.
- The downloadable alpha.21 web ZIP matches all 49 files in that public CI artifact, and the source ZIP matches all 193 raw Git blobs at commit `cc208e6`. Source packaging reads raw Git blobs to avoid platform newline normalization. Cross-OS binary reproducibility remains an explicit gap.
