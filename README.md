...
- Alpha.27 fails closed before Flutter starts whenever Harbor is embedded: the frame receives only an assertive privacy notice and an explicit “Open Harbor directly” action. The full 109-test suite, analysis, formatting, both finalized 49-asset/9-precache Wasm builds, top-level subpath rendering, cross-origin framing probe, and server-stopped offline restart pass locally. Public alpha.27 deployment and hostile-host/header-controlled production defense remain gates.
